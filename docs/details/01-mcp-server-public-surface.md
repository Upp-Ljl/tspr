# localsprite MCP Server — Public Surface (Module 01)
...
The localsprite MCP server communicates over **stdin/stdout** using newline-delimited JSON-RPC 2.0.
...
| # | Tool name |
|---|---|
| 1 | `localsprite_bootstrap_tests` |
| 2 | `localsprite_generate_code_summary` |
| 3 | `localsprite_generate_standardized_prd` |
| 4 | `localsprite_generate_frontend_test_plan` |
| 5 | `localsprite_generate_backend_test_plan` |
| 6 | `localsprite_generate_code_and_execute` |
| 7 | `localsprite_open_test_result_dashboard` |
| 8 | `localsprite_rerun_tests` |
...
### 3.1 `localsprite_bootstrap_tests`
...
### 3.2 `localsprite_generate_code_summary`

| Parameter | Type | Required | Default | Constraints |
|---|---|---|---|---|
| `projectRootPath` | string | **yes** | — | absolute filesystem path |

### 3.3 `localsprite_generate_standardized_prd`

| Parameter | Type | Required | Default | Constraints |
|---|---|---|---|---|
| `projectPath` | string | **yes** | — | absolute filesystem path |

### 3.4 `localsprite_generate_frontend_test_plan`
...
### 3.5 `localsprite_generate_backend_test_plan`

| Parameter | Type | Required | Default | Constraints |
|---|---|---|---|---|
| `projectPath` | string | **yes** | — | absolute filesystem path |

### 3.6 `localsprite_generate_code_and_execute`
...
### 3.7 `localsprite_open_test_result_dashboard`

No parameters. The input object must be present but may be empty (`{}`).

### 3.8 `localsprite_rerun_tests`
...
### 4.1 `localsprite_bootstrap_tests` success
...
### 4.2 `localsprite_generate_code_summary` success
...
### 4.3 `localsprite_generate_standardized_prd` success
...
### 4.4 `localsprite_generate_frontend_test_plan` success
...
### 4.5 `localsprite_generate_backend_test_plan` success
...
### 4.6 `localsprite_generate_code_and_execute` success
...
### 4.7 `localsprite_open_test_result_dashboard` success
...
### 4.8 `localsprite_rerun_tests` success

Same shape as §4.6 (`localsprite_generate_code_and_execute` success).
...
| `ERR_WRITE_FAILED` | 2, 3, 4, 5, 6, 7 | Cannot write to `.localsprite/` directory | Check filesystem permissions |
| `ERR_NOT_BOOTSTRAPPED` | 4 | No active session found for the given `projectPath` | Call `localsprite_bootstrap_tests` first |
...
### Group B-1: `localsprite_bootstrap_tests`

**B-1-1**: Calling `localsprite_bootstrap_tests` with a `projectPath` that does not exist
on disk returns an error response with `data.code = "ERR_PROJECT_NOT_FOUND"`.

**B-1-2**: Calling `localsprite_bootstrap_tests` with a `projectPath` that exists but
contains no `package.json` returns `data.code = "ERR_NOT_NODE_PROJECT"`.

**B-1-3**: Calling `localsprite_bootstrap_tests` with a valid Node project path and Docker
running returns a success response where `status = "ok"` and `sessionId` is a non-empty string.

**B-1-4**: The `sessionId` returned by a successful `localsprite_bootstrap_tests` call is
unique across calls (two successive calls with the same arguments return different `sessionId` values).

**B-1-5**: Calling `localsprite_bootstrap_tests` with `localPort = 0` returns an error
response with `data.code = "ERR_INVALID_PORT"`.

**B-1-6**: Calling `localsprite_bootstrap_tests` with `localPort = 65536` returns an error
response with `data.code = "ERR_INVALID_PORT"`.

**B-1-7**: Calling `localsprite_bootstrap_tests` with `localPort = 65535` (boundary) and
all other conditions met succeeds (does not raise `ERR_INVALID_PORT`).

**B-1-8**: When Docker is not reachable, `localsprite_bootstrap_tests` returns
`data.code = "ERR_DOCKER_UNAVAILABLE"`.

**B-1-9**: Calling `localsprite_bootstrap_tests` without the required `type` parameter
returns a JSON-RPC error with code `-32602` (Invalid Params).

**B-1-10**: Calling `localsprite_bootstrap_tests` with `type = "frontend"` and a valid
project returns `detectedFramework` as a non-empty string.

---

### Group B-2: `localsprite_generate_code_summary`
...
### Group B-3: `localsprite_generate_standardized_prd`
...
**B-3-5**: Calling `localsprite_generate_standardized_prd` on a project that has no
pre-existing `code_summary.json` still succeeds (the tool auto-generates the summary internally).

---

### Group B-4: `localsprite_generate_frontend_test_plan`

**B-4-1**: Calling without a prior `localsprite_bootstrap_tests` call for the given
...
### Group B-6: `localsprite_generate_code_and_execute`
...
### Group B-8: `localsprite_rerun_tests`
...
**B-8-3**: Successful call returns the same response shape as `localsprite_generate_code_and_execute`
...
**B-A-1**: After a successful `localsprite_generate_code_summary` call, the file
`{projectRootPath}/.localsprite/code_summary.json` exists and is valid JSON.

**B-A-2**: After a successful `localsprite_generate_standardized_prd` call, the file
`{projectPath}/.localsprite/standard_prd.json` exists and is valid JSON.

**B-A-3**: After a successful `localsprite_generate_frontend_test_plan` call, the file
`{projectPath}/.localsprite/frontend_test_plan.json` exists and is valid JSON.

**B-A-4**: After a successful `localsprite_generate_backend_test_plan` call, the file
`{projectPath}/.localsprite/backend_test_plan.json` exists and is valid JSON.

**B-A-5**: After a successful `localsprite_generate_code_and_execute` call,
`{projectPath}/.localsprite/test_results.json` exists, is valid JSON, and contains the
key `"status"` with one of the values `"ok"`, `"partial"`, or `"all-failed"`.

**B-A-6**: After a successful `localsprite_generate_code_and_execute` call,
`{projectPath}/.localsprite/report.html` exists and has non-zero file size.

**B-A-7**: After a successful `localsprite_generate_code_and_execute` call, at least one
`*.spec.ts` file exists under `{projectPath}/.localsprite/generated_tests/`.

**B-A-8**: After a successful `localsprite_open_test_result_dashboard` call,
`~/.localsprite/dashboard.html` exists and has non-zero file size.
...
`localsprite_bootstrap_tests` returns a JSON-RPC error with code `-32602`.

**B-V-3**: Supplying `testIds` as a non-array value in `localsprite_generate_code_and_execute`
returns a JSON-RPC error with code `-32602`.

**B-V-4**: Calling `localsprite_open_test_result_dashboard` with any input (including empty
