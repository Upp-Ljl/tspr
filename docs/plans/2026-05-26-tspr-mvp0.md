...
1. **Intent infer** — tool 2/3 / `generate_code_summary` + `generate_standardized_prd`
   ...
   - 输出：`{projectPath}/.tspr/code_summary.json` + `standard_prd.json`
...
- Docker 控制：`dockerode`
- 浏览器：`playwright`
- 状态：`better-sqlite3` → `~/.tspr/db.sqlite`（run history、test results、dashboard）
- 验证 schema：`zod`
...
└──┬──────────────────────────────────────────┘
   ...
   └─ src/tools/rerunTests.ts         (tool 8)
        │
        ├─ src/engine/llmClient.ts         (claude --model X -p)
...
1. `D:\lll\localsprite\package.json` 含 `bin: tspr`，`npm install -g .` 后 `tspr mcp` 起 MCP server
2. `dist/mcp/server.js` 真起 MCP server，stdio 双向通
3. 8 个 tool 全 implement + zod 校验输入
4. `~/.tspr/db.sqlite` 自动初始化，含 `runs` / `test_results` / `code_summaries` 三张表
5. `scripts/smoke-tspr.mjs` 端到端跑完一次：
